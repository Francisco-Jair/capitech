    ...
    public static function create_users($users) {
        ...
        foreach ($params['users'] as $user) {
            ...
            if ($DB->record_exists('user', array('username' => $user['username'], 'mnethostid' => $CFG->mnet_localhost_id))) {
                throw new invalid_parameter_exception('CPF já existe: '.$user['username']);
            }
            ...
            if (!validate_email($user['email'])) {
                throw new invalid_parameter_exception('Endereço de Email é inválido: '.$user['email']);
            } else if (empty($CFG->allowaccountssameemail) &&
                    $DB->record_exists('user', array('email' => $user['email'], 'mnethostid' => $user['mnethostid']))) {
                throw new invalid_parameter_exception('Endereço de Email já existe: '.$user['email']);
            }
