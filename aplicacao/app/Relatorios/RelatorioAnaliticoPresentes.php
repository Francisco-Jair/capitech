...
class RelatorioAnaliticoPresentes implements IRelatorio
{
	private $pdf;
	...
	public function processaRelatorio($filtro)
	{
		...
		if (is_array($filtro)) {
            ...
            $this->pdf->setTituloDoRelatorio('Relação de presença por aulas');

            $dados = Frequencia::select("aula_id")
                ->where('data_aula')
                ->distinct()->get();

            $qtd_presentes = Frequencia::where(function ($query) {
                ...
                foreach ($dados as $item) {
                    ...
                    if ($frequencias->count()) {
                        ...
                        foreach ($frequencias as $frequencia) {
                            ...
                            if ($item->presente_m == 'N')
                                $manha++;

                            if ($item->presente_t == 'N')
                                $tarde++;
            ...
            $headerPdfTable[0] = array('TEXT' => utf8_decode('QUANTIDADE DE PRESENTES'));

            $pdfTable->addHeader($headerPdfTable);
            $aRow[0] = array('TEXT' => count($qtd_presentes) . ' alunos', "TEXT_ALIGN" => "C");
