...
class RelatorioCertificadoAnaliticoAlunoPorInstituicao implements IRelatorio
{
	private $pdf;
	...
	public function processaRelatorio($filtro)
	{
		...
		if (is_array($filtro)) {
			...
			$dados = Aluno::select('firstname', 'lastname', 'email', 'username', 'institution')
				->join('certificados', 'alunos.id', '=', 'certificados.user_id')
			...
			$this->pdf->setTituloDoRelatorio('Relação de alunos com certificado por instituição');
			...
			if (!empty($dados)) {
			   ...
			   * */
					$pdfTable->initialize(
						[1.5, 7, 9.8], $this->pdf->getConfiguracaoPadraoPdfAdvanced()
					);
					...
					$headerPdfTable2[2] = ['TEXT' => 'ASSINATURA'];
