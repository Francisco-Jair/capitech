...
class Pdf_Table {
    ...
    /**
     * Padding Top.
     * Numeric. Expressed in units. @example: 5
     */
        ...
        for ( $i = 0; $i < $this->nColumns; $i ++ ) {
            ...
            if ( ! empty( $aLastDataCache ) ) {
                ...
                if ( isset( $aLastDataCache['DATA'][$i] ) && ($aLastDataCache['DATA'][$i]->ROWSPAN > 1) ) {
                    ...
                    $oCell->setRowSpan( $aLastDataCache['DATA'][$i]->getRowSpan() - 1 );
                    $oCell->setRowspanId( $aLastDataCache['DATA'][$i]->getRowSpanId() );
                    $oCell->setRowspanPrimary( false );
                    
                    //copy the colspan from the last value
                    $oCell->setColSpan( $aLastDataCache['DATA'][$i]->getColSpan() );
                    
                    //cell width is the same as the one from the line before it
                    $oCell->setCellDrawWidth( $aLastDataCache['DATA'][$i]->getCellDrawWidth() );
            ...
            if ( $oCell->getRowspan() > 1 ) {
                ...
                $oCell->setRowspanPrimary( true );
                $oCell->setRowspanId( $this->iRowSpanID );
                $aRowSpan[] = $i;
            }
    ...
    protected function _cacheParseRowspan ( $iStartIndex = 0, $sCacheType = 'data' ) {
        ...
        for ( $ix = $iStartIndex; $ix < $iItems; $ix ++ ) {
            ...
            foreach ( $val['ROWSPAN'] as $k ) {
                ...
                for ( $i = 0; $i < $val['DATA'][$k]->getRowSpan(); $i ++ ) {
                ...
                if ( $h_cell > $h_rows ) {
                    ...
                    $add_on = ($h_cell - $h_rows) / $val['DATA'][$k]->getRowSpan();
                    for ( $i = 0; $i < $val['DATA'][$k]->getRowSpan(); $i ++ ) {
                        if ( isset( $aRefCache[$ix + $i] ) ) {
                    ...
                    case 1:
                        ...
                        if ( true == $bSplitCommand ) {
                            ...
                            for ( $j = 0; $j < $this->nColumns; $j ++ ) {
                                 ...
                                 */
                                if ( ($aData[$j]->getSkipped() === TRUE) || ($aData[$j]->getRowSpan() > 1) )
                                    continue;
                                ...
                                if ( $aData[$j]->ROWSPAN > 1 ) {
                                    //DebugBreak();
                                }
                                
                                if ( $aData[$j]->isSplittable() ) {
                                    list ( $aTData[$j] ) = $aData[$j]->split( $val['HEIGHT'], $iLeftHeightLast );
                                    $aData[$j]->setCellDrawHeight( $iLeftHeightLast );
                                } else {
                                    $aTData[$j] = clone $aData[$j];
                                    
                                    $o = new Pdf_Table_Cell_Empty( $this->oPdf );
                                    $o->copyProperties( $aData[$j] );
                                    $o->setCellDrawWidth( $aData[$j]->getCellDrawWidth() );
                                    $o->setCellHeight( $iLeftHeightLast );
                                    $aData[$j] = $o;
                                }
                                
                                $fRowH = max( $fRowH, $aData[$j]->getCellDrawHeight() );
                                $fRowHTdata = max( $fRowHTdata, $aTData[$j]->getCellDrawHeight() );
    ...
    protected function _cachePrepOutputData () {
        ...
        for ( $k = 0; $k < $iItems; $k ++ ) {
            ...
            for ( $i = 0; $i < $this->nColumns; $i ++ ) {
                ...
                if ( $data[$i]->getSkipped() === FALSE ) {
                    
                    //render the cell to the pdf
                    //$data[$i]->render($rowHeight = $val['HEIGHT']);
                    

                    if ( $val['HEIGHT'] > $data[$i]->getCellDrawHeight() ) {
                        $data[$i]->setCellDrawHeight( $val['HEIGHT'] );
                    }
    ...
    protected function getDefaultConfiguration () {
        ...
        require dirname( __FILE__ ) . '/../configTable.php';
