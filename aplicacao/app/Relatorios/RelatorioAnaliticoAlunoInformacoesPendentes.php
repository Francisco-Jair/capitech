...
class RelatorioAnaliticoAlunoInformacoesPendentes implements IRelatorio
{
    private $pdf;
    ...
    public function processaRelatorio($filtro)
    {
        ...
        if (is_array($filtro)) {
            ...
            $dados = $dados->get();
            $this->pdf->setTituloDoRelatorio('Relação de alunos com informações pendentes');
...
//                    $pdfTable->addHeader($headerPdfTable3);
                    ...
                    $alunos = Matricula::where(function ($query) {
                        $query->where('email','like','student')
                            ->orWhere('username', 'like', '000000%');
                    })
